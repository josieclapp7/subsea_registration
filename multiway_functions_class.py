...
class MultiwaySolver:
    ...
    def _pairwise_registration(self, source: o3d.geometry.PointCloud, target: o3d.geometry.PointCloud, source_id: int, target_id:int) -> ScanPair:
        '''Coarse then fine ICP'''
        cfg = self.config
 
        if cfg.use_point_to_plane:
            print("Apply point-to-plane ICP")
            icp_coarse = o3d.pipelines.registration.registration_icp(
                source, target, cfg.coarse_distance, np.identity(4),
                o3d.pipelines.registration.TransformationEstimationPointToPlane())
            icp_fine = o3d.pipelines.registration.registration_icp(
                source, target, cfg.fine_distance,
        ...
        information_icp = o3d.pipelines.registration.get_information_matrix_from_point_clouds(
            source, target, cfg.fine_distance, icp_fine.transformation)
        ...
        pair = ScanPair(
            ...
            information=information_icp,
            inlier_rmse=icp_fine.inlier_rmse,
            is_adjacent=(target_id == source_id + 1),
        )
        self.scan_pairs.append(pair)
        return pair
    
    def _full_registration(self, pcds: list) -> o3d.pipelines.registration.PoseGraph:
        '''Loop over all scan pairs, run pairwise ICP, and build the pose graph'''
        ...
        for source_id in range(len(pcds)):
            for target_id in range(source_id + 1, len(pcds)):
                pair = self._pairwise_registration(pcds[source_id], pcds[target_id], source_id, target_id)
                print("Build Pose Graph")

                if pair.is_adjacent:  # odometry (direct neighbors)
                    odometry = np.dot(pair.transformation, odometry) #this is the cumulative transformation
                ...
                pose_graph.edges.append(
                    o3d.pipelines.registration.PoseGraphEdge(source_id, 
                                                             target_id, 
                                                             pair.transformation, 
                                                             pair.information, uncertain = False))
        return pose_graph #contains nodes (scan poses) and edges (transformations and weights)
    
def load_point_clouds_down(pcd_files: list, config:RegistrationConfig) -> list:
    '''
    ...
    for path in pcd_files:
        ...
        pcd_down = cloud.voxel_down_sample(voxel_size=config.voxel_size)
...
def compute_global_rmse(pcds_down: list, pose_graph, threshold: float = 0.009, adjacent_only = True) -> tuple:
    '''
...
def color_from_name(name: str) -> list:
